...
if (line === "[fadeIn]") {
    inputLocked = true;
    // 這裡我們利用一個全黑的覆蓋層來實作，或直接操控 game-container
    await screenFade(0); // 這裡的 0 代表透明度變為 0 (看到畫面)
    inputLocked = false;
    currentIndex++;
    return processLine();
}
...
if (line === "[fadeOut]") {
    inputLocked = true;
    await screenFade(1); // 這裡的 1 代表透明度變為 1 (變全黑)
    inputLocked = false;
    currentIndex++;
    return processLine();
}
...
async function screenFade(opacityValue) {
    // 假設我們在 HTML 有一個 id 為 "fade-layer" 的黑底層
    let layer = document.getElementById("fade-overlay");
    if (!layer) {
        layer = document.createElement("div");
        layer.id = "fade-overlay";
        layer.style.cssText = "position:absolute;top:0;left:0;width:100%;height:100%;background:black;z-index:9999;pointer-events:none;transition:opacity 1.5s;";
        document.getElementById("game-container").appendChild(layer);
    }

    layer.style.opacity = opacityValue;
    await wait(1500); // 等待動畫完成
}
