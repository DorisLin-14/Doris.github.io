...
function hideAllPages() {
    ['portfolio-page', 'works-page'].forEach(id => {
...
function showProductDetail(title, desc, url, imgSrc) {
    ...
    if (imgSrc) {
        ...
        // 清除舊動畫類名，強制重啟動畫
        imgElement.classList.remove('anim-fade-in', 'anim-page-flip');
        void imgElement.offsetWidth; // 關鍵：強制瀏覽器重繪 (Reflow)
        ...
        if (title.includes("聰明白貓")) {
            ...
            // 預設 (包含呆橘貓) 使用淡入
            imgElement.classList.add('anim-fade-in');
        }
    }
    
    // 3. 自動捲動至詳情區
    document.getElementById('product-detail-area').scrollIntoView({
        behavior: 'smooth',
        block: 'center'
    });
}
